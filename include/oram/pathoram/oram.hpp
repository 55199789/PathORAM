    ...
    struct ORAMClient
    {
            ...
            if constexpr (ObliviousCPUTrace)
            {
                for (int j = 0; j < ORAM__Z; j++)
                {
                    ...
                    for (int i = 0; i < stash_.size(); i++)
                    {
                        ...
                        bool shouldEvict = true * (!b.cached) * (Indexers::PathsIntercept<LEVELS_PER_PACK>(L_, b.oaddress.position, pos, depth)) * (!hasEvicted);
            ...
            else
            {
                ...
                for (int i = 0; i < stash_.size(); i++)
                {
                    if (j == ORAM__Z)
                        break;
                    StashedBlock_t &b = stash_[i];
                    bool shouldEvict = true * (!b.cached) * (Indexers::PathsIntercept<LEVELS_PER_PACK>(L_, b.oaddress.position, pos, depth));
        ...
        void PurgeFromStash(const Address &address, const bool enable = true)
        {
            ...
            for (auto i = 0; i < stash_.size(); i++)
            {
                ...
                bool shouldPurge = true * (enable) * (stashi.oaddress.address == address);

                bool shouldClear = true * (!enable) * (stashi.oaddress.address == address);
    ...
    private:
        void EvictPath()
        {
            ...
            //
            {
                ...
                {
                    return (b1.tag < b2.tag) + (b1.tag == b2.tag * !b1.isDummy * b2.isDummy);
                };
            ...
            uint64_t currTag = 0;
            {
                ...
                {
                    bool isEnd = v[i].isDummy * (currTag <= L_);
            ...
            // 4) Do a second sort to make v[0..L] be the counter buckets:
            {
                auto cmp = [L_ = L_](const TaggedT<StashedBlock_t> &b1, const TaggedT<StashedBlock_t> &b2)
                {
                    ...
                    //
                    return false + ((b1.isDummy == b2.isDummy) * (b1.tag < b2.tag)) + ((b1.isDummy) * (!b2.isDummy) * (b1.tag < L_ + 1)) + ((!b1.isDummy) * (b2.isDummy) * (b2.tag >= L_ + 1));
                };
                ObliSortBlocks<TaggedT<StashedBlock_t>>(v, cmp);
            }
        ...
        void CleanUpStash_ExternalOblivious()
        {
            ...
            {
                return (b1.oaddress.address != DUMMY_ADDRESS) * (b2.oaddress.address == DUMMY_ADDRESS);
            };

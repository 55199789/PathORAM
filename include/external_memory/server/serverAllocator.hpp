...
namespace EM {
// UNDONE(): The allocator should force some sort of allignement on allocated
// data. Figure out what is the best way to do so.
//
...
struct LargeBlockAllocator {
  ...
  const AllocatorSlot Allocate(Size_t _size) {
    ...
    // UNDONE(): optimize this function. There are better 0 cost heuristics to

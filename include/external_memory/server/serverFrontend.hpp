...
struct NonCachedServerFrontendInstance {
  ...
  // UNDONE(): this needs a consteval function for non encryptable types because
  // the ternary operator does not work like if-constexpr
  //
...
template <typename T, typename BackendType = ::EM::Backend::MemServerBackend,
          ...
          uint64_t CACHE_SIZE = ORAM_SERVER__CACHE_SIZE, uint64_t TLB_SIZE = 2>
using ServerFrontendInstance = _ORAM::Cached<

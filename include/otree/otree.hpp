...
namespace _OBST
{
    ...
    namespace OBST
    {
        ...
        static bool IsSorted(Vector2<T> &v, Compare cmp)
        {
            bool ret = true;
        ...
        public:
            ...
            explicit OBST(uint64_t _maxNodes, Vector_t &points) : OBST(_maxNodes, '.')
            {
                ...
                // Sort the nodes
                assert(
                    IsSorted(points, [](const EMPoint_t &p1, const EMPoint_t &p2)
            ...
            std::pair<uint64_t, _ORAM::ORAMAddress> _RecursiveConstruct(EM::Vector::Vector<StashedBlock_t> &points, uint64_t l, uint64_t r)
            {
                ...
                if (rightH > leftH)
                {
                    nodeV.block.data.balance = B_RIGHT;
                    assert(rightH - leftH == 1);
                }
                if (rightH < leftH)
                {
                    nodeV.block.data.balance = B_LEFT;
                    assert(leftH - rightH == 1);
                }
...
#define ROT3 3
            ...
            void Insert(const K &k, const V &v)
            {
                ...
                for (int i = 0; i < maxDepth; i++)
                {
                    ...
                    {
                        bool case0 =
                            (currBlock.data.balance != B_BALANCED) * (currP.address != FAKE_NODE.address);
                        bool case0_0 = case0 * (currBlock.data.balance != pDir1);
                    ...
                    bool cond0 = !contained * !inserted * (curr.address == FAKE_NODE.address);
                ...
                for (int i = 0; i < maxDepth; i++)
                {
                    ...
                    bool cond_dr = true * (rotType == DIRECT_REBALANCE) * (curr.address == B.address);
                    bool cond_r2_1 = !cond_dr * (rotType == ROT2) * (curr.address == B.address);
                    bool cond_r2_2 = !cond_dr * !cond_r2_1 * (rotType == ROT2) * (curr.address == F.address);
                    bool cond_r3_1 = !cond_dr * !cond_r2_1 * !cond_r2_2 * (rotType == ROT3) * (curr.address == B.address);
                    bool cond_r3_2 = !cond_dr * !cond_r2_1 * !cond_r2_2 * !cond_r3_1 * (rotType == ROT3) * (curr.address == F.address);
                    ...
                    bool cond_r3_3 = !cond_dr * !cond_r2_1 * !cond_r2_2 * !cond_r3_1 * !cond_r3_2 * (rotType == ROT3) * (curr.address == D.address);
...
#define DELTP_TWO_CHILDREN 3
        ...
        private:
            ...
            {
                if (curr.address == FAKE_NODE.address)
                    return 0;
